...
- Wiring the *remaining* real texture consumer (the pulse, `CYBERPUN-17-10`) into an actual on-screen scene — `TextureLoading.texture(named:)` exists and is tested, but no scene mounts that sprite yet. `CYBERPUN-17-10-t1` (PR #46) landed the decision layer only (`PulseAbility` + `LevelScaling`, no production caller). `CYBERPUN-17-10-t2` landed the HUD pulse button component (`Sources/UI/PulseButton.swift`; `PulseButtonTests`): a self-contained, presentation-only `TouchResponder` node — no dependency on `PulseAbility`/`GameScene` — exposing `setCooldownProgress(_:)`/`setOnCooldown(_:)` to drive an externally-tracked cooldown visual and firing its `onPress` closure on every press regardless of cooldown state (product gate 1's "must respond to every press"); sized to `FloatingThumbstickNode.pulseButtonSlotSize`, the slot a later PR mounts it in, but **not yet mounted anywhere** — no `GameScene` mount, no `PulseAbility` wiring, no live `sprite_pulse` ring. The scene wiring that makes product gate 8 demonstrable on a running simulator is now filed as **`CYBERPUN-17-10-t3`** — this story's own next task, following the same `-t3` scene-wiring precedent as `CYBERPUN-17-9-t3` and `CYBERPUN-17-11-t3`. Its scope: mount `PulseButton` at `FloatingThumbstickNode.reservedPulseButtonSlot(forSize:safeAreaInsets:)` — positioning from the slot's *centre* (`slot.midX`/`slot.midY`), never `slot.origin`, since the node draws its plate centred on its own origin — hide it outside `.gameplay` (it is an accessibility element with no `isRunActive`-style gate of its own, so an unhidden mirror would sit over the menu's bottom-left quadrant and forward touches into `dispatchTouch`), drive `setCooldownProgress(_:)` from `PulseAbility.cooldownRemaining`/`cooldownSeconds` in the scene's per-frame hook, and mount the live `sprite_pulse` ring. Until `-t3` lands, **nothing in `CYBERPUN-17-10` is reachable from a running simulator** and product gate 8 cannot be demonstrated: `-t1` and `-t2` both test in isolation from `GameScene`, so the suite stays green while the gate stays unmet. Also awaiting a human tuning call, not an engineering one: the level-6 radius bonus ships as *compounding* (1.25 * 1.25 == 1.5625) rather than additive (1.5), so whoever owns gate 8's tuning should confirm the reading. Bullets, the muzzle flash and the hit puff are no longer covered by this bullet: CYBERPUN-17-9-t3 mounted `BulletPool`/`HitEffects` under `GameScene.effectsLayer` — read the CYBERPUN-17-9 entry below for their real state. Pickups are no longer covered either: CYBERPUN-17-11-t3 landed the production mount and player/raccoon collision wiring — read the CYBERPUN-17-11 entry below for their real state. Ground tiles, buildings and rooftop signs (CYBERPUN-17-5-t2/-t3, `GroundPlaneStreamer.mountChunk` into `GameScene.worldLayer`), the player actor (CYBERPUN-17-6) and the raccoon swarm (CYBERPUN-17-8-t2, `RaccoonSpawnDirector`) are no longer deferred, so product gate 4 is reachable in a real build — precisely the justification for the CYBERPUN-17-5-t4 placeholder-label removal described below
...
- Per-run `worldSeed`: **unmet** product gate 6 ("every run starts at a new junction") - nothing writes it (`GameScene.spawnTilePosition()`); asked on `-t3`/`-t4`/`-t5`, still NO TICKET ID - file one here before closing the story
...
- Local high-score persistence; the SCAFFOLDING marker grep gate; audio, app icon art, launch screen polish, App Store metadata/submission
