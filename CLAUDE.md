...
- Ground pickups (`CYBERPUN-17-11`) — **the authoritative implemented-vs-outstanding list for this story; read this rather than inferring the remaining scope from `PickupKind`/`Pickup`/`PickupManager`/`PickupNode`'s doc comments.**
  ...
  Outstanding, no invented ticket IDs: **no production caller** — nothing constructs a `PickupManager` or mounts a `PickupNode`, so no pickup can appear in a real build and product gate 5 ("first spawn <=10s in normal play", "icons legible as a med kit / garbage can at a glance") is unverifiable on a device; applying a collected med kit's 1d10 roll to `PlayerNode.hp`; diverting a wounded raccoon (`RaccoonNode.isWounded`) to a garbage can; and the `maxAlive: 2` cap, unreachable at the frozen tuning (cadence 25s > lifetime 20s holds the effective ceiling at 1 per kind — recorded on `PickupKind.Tuning.maxAlive`, pinned by `PickupManagerTests.test_maxAlivePerKind_isNeverExceeded_andTheObservedCeilingIsPinned`), which needs a decision from whoever tuned it
