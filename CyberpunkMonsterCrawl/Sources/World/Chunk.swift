...
final class Chunk {
    ...
    /// The buildings `ChunkGenerator.generate` placed on the blocks this
    /// chunk **owns** (`BuildingPlacement.blockCoordinates(ownedByChunk:)` —
    /// every block whose interior lower corner lands in this chunk, so each
    /// block in the plane is generated by exactly one chunk). A block whose
    /// interior straddles a chunk seam is therefore fully generated by its
    /// owner, and some of these records' `footprintTiles` legitimately fall
    /// in the neighbouring chunk; use `reservedTiles` (which reads through
    /// the world-lifetime store) rather than these records to ask "is this
    /// tile occupied".
    ///
