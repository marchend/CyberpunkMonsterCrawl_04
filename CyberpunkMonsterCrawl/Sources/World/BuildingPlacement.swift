...
struct BlockCoordinate: Hashable {
    ...
    /// The single chunk responsible for generating this block's buildings:
    /// the chunk containing the block interior's lower-corner tile.
    ///
    /// Every block in the plane has exactly one owning chunk, which is what
    /// makes `BuildingPlacement.blockCoordinates(ownedByChunk:)` a partition
    /// rather than a filter — a block whose interior straddles a chunk seam
    /// is generated by its owner and by nobody else, instead of being
    /// dropped by both sides.
    var owningChunk: ChunkCoordinate {
        ChunkCoordinate.containing(tileX: interiorOrigin.tileX, tileY: interiorOrigin.tileY)
    }
}
...
enum BuildingPlacement {
    ...
    /// Every block **owned by** `chunkCoordinate` — the blocks whose 3x3
    /// interior lower-corner tile (`BlockCoordinate.interiorOrigin`) falls
    /// inside this chunk's own 8x8 world-tile footprint, i.e. exactly the
    /// blocks whose `owningChunk` is this chunk.
    ///
    /// **Ownership, not containment.** An earlier rule here offered only
    /// blocks whose *entire* interior sat inside one chunk, borrowing
    /// `Chunk.reservableFootprints(in:)`'s no-cross-chunk-lookup discipline.
    /// That discipline does not apply: `reservableFootprints(in:)` must
    /// consult `tiles` for a `TileKind` it can only see inside its own
    /// chunk, whereas `generate(forBlock:seed:)` reads nothing but
    /// `(block, seed)` and has no cross-chunk lookup to avoid. The cost of
    /// borrowing it was severe — `Chunk.size` (8) is not a multiple of
    /// `CityLatticeGenerator.period` (6), so per axis only 3 of every 4
    /// block interiors lie wholly within a single chunk (interiors at tiles
    /// 0-2, 6-8, 12-14, 18-20 against chunk spans 0-7, 8-15, 16-23: the 6-8
    /// interior straddles the seam). In 2D that is ~9/16, so ~44% of
    /// building blocks were resolved by *neither* neighbour and carried no
    /// buildings at all: holes in the city, not variety.
    ///
    /// Naming a single deterministic owner keeps the wider net
    /// duplicate-free. Every block in the plane is owned by exactly one
    /// chunk, so sweeping any set of chunks yields each block exactly once,
    /// and a block whose footprint tiles spill across a seam is still fully
    /// generated (by its owner). `ChunkGeneratorTests
    /// .test_blockCoordinates_ownedByChunk_partitionTheBlocksTheChunksCover`
    /// pins the partition in both directions: no block dropped, none
    /// emitted twice.
    static func blockCoordinates(ownedByChunk chunkCoordinate: ChunkCoordinate) -> [BlockCoordinate] {
        let origin = chunkCoordinate.worldTileOrigin
        let xs = axisBlockCoordinates(ownedByChunkOriginAxis: origin.tileX)
        let ys = axisBlockCoordinates(ownedByChunkOriginAxis: origin.tileY)
    ...
    /// The block coordinates, on a single axis, whose interior *start*
    /// (`b * period`) falls within `[chunkOriginAxis, chunkOriginAxis +
    /// Chunk.size - 1]` — the one-axis half of the `owningChunk` rule.
    ///
    /// A span of `Chunk.size` (8) consecutive tiles always contains at
    /// least one and at most two multiples of `period` (6), so a chunk
    /// always owns 1-2 block indices per axis (1-4 blocks in total) and
    /// never zero.
    private static func axisBlockCoordinates(ownedByChunkOriginAxis chunkOriginAxis: Int) -> [Int] {
        ...
        for candidate in lowerBound...upperBound {
            ...
            if interiorStart >= chunkOriginAxis && interiorStart <= chunkOriginAxis + Chunk.size - 1 {
