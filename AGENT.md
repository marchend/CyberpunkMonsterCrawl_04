...
CyberpunkMonsterCrawl/
  ...
  GameScene.swift                          three persistent layers (worldLayer/effectsLayer/uiLayer), state-driven screen registry, UI-first touch dispatch; mounts the streamed ground plane + PlayerNode on entry to .gameplay, snapping the player's screen position via PixelCrispness.snappedPosition on every mount/reposition; mounts the floating thumbstick in uiLayer and routes touchesBegan/Moved/Ended/Cancelled to it (activeStickTouch); runs the per-frame run pipeline in advanceMovementAndCamera(currentTime:): thumbstick -> PlayerMovementController -> CollisionResolver -> PlayerNode position/depth/visual state -> CameraController (CYBERPUN-17-7). The SCAFFOLDING(CYBERPUN-17-7) PlayerScaffoldingDriver demo vector, its debugPlayerDemoEnabled flag and the debug camera pan are deleted. CYBERPUN-17-12 PR 2: mounts HUDLayer (Sources/UI/HUD/) in uiLayer on the first .gameplay entry - reused across RUN AGAIN, hidden outside a run - lays it out from didChangeSize/didMove/refreshLayoutForCurrentSafeArea AND again as the run becomes interactive, binds it to this scene's own HUDRunModel conformance (an extension of computed reads off player/playerCombat/runElapsedSeconds/runStats/pulseAbility, no cached copy), calls hudLayer.refresh() + evaluateSwarmEscalation() every .gameplay frame, and routes the HUD button's press into the same handlePulsePress() the older bottom-left pulseButton uses - which, since PR #63's review decision, is hidden in every state (constructed and wired, never shown), leaving the HUD's bottom-right button as the run's only visible ability control until CYBERPUN-17-14 deletes the older mount
  ...
  Screens/GameplayScreenNode.swift         skeleton .gameplay screen; mounts no full-bleed backdrop and, since CYBERPUN-17-5-t4, no text of its own, and (since CYBERPUN-17-7-t5) no accessibility marker either — the streamed city + player render through it, world touches fall through, and the screen still mounts no children at all - the real HUD (CYBERPUN-17-12) is GameScene.hudLayer, a direct uiLayer child and a *sibling* of this screen rather than its content, the same mount convention thumbstick/pulseButton follow
...
CyberpunkMonsterCrawlTests/              CYBERPUN-17-12's HUD suites: HUDLayoutTests + HUDElementUpdateTests (PR 1 - pure slot geometry and each element's own update(...); PR 2 added the missing "no two slots overlap EACH OTHER" gate, which is what caught PR 1's centred run timer being drawn through the HP bar in portrait), then HUDRotationUITests (portrait -> landscape -> portrait mid-run against a fixed-inset SKView: every mounted element inside the safe content rect and mutually disjoint, HUD hidden once the run ends, exactly one HUDLayer ever mounted), HUDThumbstickOverlapTests (nothing in the region the stick OCCUPIES; the pulse button wholly outside the stick's whole-left-half touch-ACCEPTANCE box, corners included; a HUD touch reaches the button and fires the real ability), HUDRunModelBindingTests (bind/refresh forward live values with no stale cache, rebinding retargets the trigger AND clears the old model's onSwarmEscalation hook while a rebind to the same model keeps it) and SwarmBannerEscalationTests (escalation shows the banner, it self-dismisses, input still reaches the HUD and the stick while it shows, plus GameScene.swarmEscalationTier's banding). Since PR #63 review every landscape assertion in HUDLayoutTests/HUDRotationUITests/HUDThumbstickOverlapTests runs at BOTH 844x390 and the tightest supported 812x375 / bottom-21 geometry, and AccessibleSKViewTests.test_duringARun_theHUDPublishesOnlyItsPulseButton pins the elements a live run publishes to exactly one - the HUD's bottom-right gameplay.hudPulseButton, the only visible ability control since PR #63's review decision hid the older bottom-left mount - so no interactive mirror can appear over a passive read-out inside the thumbstick's acceptance box
...
CyberpunkMonsterCrawlUITests/
  CyberpunkMonsterCrawlUITests.swift       menu present + PLAY hittable + tapping it unmounts the whole menu (PLAY *and* the durable menu.container anchor) with the app still foregrounded; it does NOT prove the destination screen end-to-end (GameplayScreenNode itself still mounts no anchor; since CYBERPUN-17-12 the HUD's gameplay.hudPulseButton is a durable accessible anchor in uiLayer, but this test was deliberately not re-pointed at it) - PLAY -> .gameplay is pinned in-process by GameViewControllerCompositionTests
...
.mothership/journeys/*.json                product-verification journeys, one per story with a user-visible state; each names its story in "stories" (the gate only runs a journey for the story it names) and carries the reviewer's brief in "demonstrates". menu-to-gameplay (CYBERPUN-17-5): launch -> screenshot the menu -> navigate to PLAY (plain language, located visually) -> screenshot the streamed city with no placeholder text over it. raccoon-swarm (CYBERPUN-17-8): ~30s/~50s waits so the swarm has crossed the off-screen spawn gap and reached the bite standoff ring. pickup-spawn (CYBERPUN-17-11): ~12s/~18s waits for a bobbing pickup on a street tile (pinned by JourneyManifestTests.test_aJourneyExistsForThePickupSpawnWork_andCapturesPastTheFirstSpawnDelay: a journey must name the story and its first gameplay screenshot must land at or past PickupKind's 8s firstSpawnDelay, or the frame is an empty street) -- the probe has twice reported the process gone shortly after that delay fires; the assert_running bisection checkpoints -t6 added around it were removed again by CYBERPUN-17-14-t1 as diagnostic instrumentation with no filed removal owner, and what is known about that crash now lives in this file's CYBERPUN-17-11 entry instead. auto-fire-weapons (CYBERPUN-17-9): the weapon overlay + the swarm in weapon range, and deliberately NO bullet frame (AC1 gates fire on movement; the probe has no drag verb). death-and-high-scores (CYBERPUN-17-13): a derived floor wait (JourneyManifestTests.secondsBeforeThePlayerCanBeDead, ~31s) and then wait_for_element on the RUN AGAIN button (mounts only once .death is entered) rather than a guessed HP-to-zero duration, screenshots the eight-row death summary, chains a second death off the RUN AGAIN run exiting via BACK TO MENU into HIGH SCORES, and screenshots the table rendered from the live store -- relaunch persistence is not probe-visible (no cold start) and stays owned by HighScoreStoreTests, as AC7 orientation/overlap (no rotate verb in the probe vocabulary) stays owned by ScreensTests. gameplay-hud (CYBERPUN-17-12): two captures ~20s apart of the assembled HUD over a live run, so the run clock is visibly ADVANCED in the second (a HUD painted once with static numbers looks identical in a single frame); a rising KILLS count and the SWARM INCOMING banner are deliberately NOT promised (auto-fire needs movement and the probe has no drag verb; the banner self-dismisses), and landscape is not covered at all - the rotation claim stays owned by HUDRotationUITests plus the PR's attached screenshots
```
...
- Final gameplay HUD, death-screen run-summary rows and the high-scores list
  ...
  existed to re-point them at; `GameplayScreenNode` still mounts no children
  at all. Its `layout(for: safeAreaInsets:)` is deliberately a no-op
- Gameplay HUD (`CYBERPUN-17-12`) - **implemented and live.** PR 1 shipped the six elements + `HUDLayout`; PR 2 shipped `HUDLayer` (the composition root), `GameScene`'s `HUDRunModel` conformance and the mount/bind/refresh/re-layout wiring, so a run shows real HP, level/XP, an advancing mm:ss clock, the kill count and a working pulse button, all read from the run model every `.gameplay` frame with no cached copy anywhere. Two defects PR 1's own gates could not see were fixed in PR 2 rather than shipped: its slots were each checked against a *fixed* rect but never against **each other**, so in portrait the centred run timer was drawn straight through the 220pt HP bar (and the kill count through the timer, and the banner through the level/XP bar) - the top row is now two edge-anchored columns with the banner hung below both, and `HUDLayoutTests`/`HUDRotationUITests` pin mutual disjointness in both orientations. **One visible ability button, decided by the human reviewer on PR #63:** the HUD's bottom-right `HUDPulseButton` is the run's only visible ability control - the placement the ticket asks for - and `CYBERPUN-17-10`'s older bottom-left `PulseButton` mount is now hidden in **every** state, `.gameplay` included (`GameScene.updateWorldContent(for:)`). That node is still constructed, laid out, kept in step with the cooldown and wired to the same `handlePulsePress()`, so nothing is dead or placeholder; hidden simply means unroutable and unpublished (`isVisibleToTouchRouting(_:)`/`isVisible(_:upTo:)` filter `isHidden`). *Deleting* it, and with it `FloatingThumbstickNode.reservedPulseButtonSlot`'s hole in the stick's touch-acceptance box, is left to `CYBERPUN-17-14`. **Outstanding, no invented ticket IDs:** until that lands the reserved slot is a 72x72 patch where neither the stick nor any visible button takes the touch (the stick still refuses it - pinned by `PulseSceneWiringTests.test_theThumbstick_refusesATouchOnThePulseButtonsSlot`, whose doc note records the trade), because removing the reservation is `CYBERPUN-17-14`'s consolidation rather than a HUD-wiring change. The `.mothership/journeys/pulse-ability.json` navigate step and `PulseAbilityLiveCompositionTests`' two live-press journeys were re-aimed at the bottom-right button, since the bottom-left one is no longer on screen or reachable. Also outstanding: the rabies indicator (`CYBERPUN-17-8`'s), `swarmEscalationRaccoonStep`/`swarmEscalationDuration` are untuned first guesses, and landscape has no probe evidence (no rotate verb) - it is owned by `HUDRotationUITests` and the PR's attached screenshots. **PR #63 review round:** four defects/asymmetries the review raised were fixed rather than argued away, and one question was escalated instead of guessed. (1) `HUDLayout.swarmBannerSize` had been *tuned* (44 -> 36) against the landscape top-stack budget computed at one hardcoded inset pair (844x390, top 0 / bottom 21): that left 5pt of slack there and went ~10pt **negative** on a 375pt-tall landscape carrying the same 21pt home indicator (iPhone X/XS/11 Pro, 12/13 mini), because the constraint is `H >= 364 + top + bottom` == 385pt on a 375pt screen - so the banner drew *inside* `thumbstickReservedRegion`, the exact rect `HUDLayoutTests` exists to pin. `frame(for:)`'s `.swarmBanner` case now derives the banner's **placement** from that budget (`max(hungBelowBothColumns, clearOfTheStick)`) rather than its height from one device's insets, and `HUDLayoutTests`/`HUDRotationUITests`/`HUDThumbstickOverlapTests` each carry an 812x375 / bottom-21 entry so the tightest supported geometry is exercised, not assumed. (2) `GameScene.isVisible(_:upTo:)` (touch routing) filtered only `isHidden` while `isVisibleToTouchRouting(_:)` (the accessibility walk, documented as having to agree with it) also filtered `alpha <= 0`; routing now applies that one predicate, which is load-bearing for `SwarmBanner` - a transparent 280x36 plate spends most of a run hung over the middle of the screen. (3) The accessibility mirror the review asked about was **real, and the first fix for it was not enough** - which is why the assertion was written before the claim. Setting `isAccessibilityElement = false` explicitly on the five passive HUD elements *and* every drawn child of theirs still left SpriteKit publishing the three *visible* `SKLabelNode`s (the level/XP caption, the run clock, the kill count) during a live run: `AccessibleSKViewTests.test_duringARun_theHUDPublishesOnlyItsPulseButton` failed 4-published-vs-1-expected on a real host test run, and the banner's label escaped only because the banner is invisible most of a run. The per-class implicit value wins over an explicit `false` for that node class (each label also has its `text` re-set by its own `update(...)`, the likeliest re-derivation point - unverified, and nothing rests on it). Fixed at the cause instead: `Layers/AccessibilityOpaqueNode.swift` (new marker protocol) declares a node whose **whole subtree** publishes nothing, `GameScene.collectAccessibleNodes(under:into:)` skips a conformer without consulting any descendant's flag, and `HPSegmentBar`/`LevelXPBar`/`RunTimerLabel`/`KillCountLabel`/`SwarmBanner` conform (their per-node flags stay as intent-at-the-node, explicitly documented as a second line of defence rather than the binding one). So `AccessibleSKView` can no longer lay an interactive mirror - which wins UIKit's hit test for its rect and forwards only the `.began` phase - over the HP / level-XP bars sitting inside the floating stick's touch-acceptance box. The run-level assertion above plus `test_accessibleUINodes_skipsAnOpaqueSubtree_evenWhenAChildOptsIn` (the walk rule itself, hardest case: a descendant that opts in) pin both halves. (4) `HUDLayer.bind(to:)` clears the *previously* bound model's `onSwarmEscalation` (identity-checked, so `GameScene` rebinding to itself is left alone), so a rebind cannot leave an old model holding a live hook into this layer's banner - `HUDRunModelBindingTests` pins both halves. **Escalated, then settled by the human reviewer:** the two-mounted-ability-buttons interim state was raised on PR #63 as needing an explicit human decision (hide the older bottom-left mount now vs. ship both until `CYBERPUN-17-14`). The answer was *hide it now*, and that is what ships: `pulseButton.isHidden = true` in the `.gameplay` branch as well as outside it, the node kept constructed, and `AccessibleSKViewTests.test_duringARun_theHUDPublishesOnlyItsPulseButton`'s expected published set narrowed to just `gameplay.hudPulseButton`. Three suites moved with it rather than being left to fail or to assert the old intent: `PulseSceneWiringTests`' visibility case now pins "hidden in every state" (with its own note on why the expectation inverted) and its routing gate now dispatches at the HUD button, plus a new case pinning that the hidden older slot claims no touch and fires nothing; `PulseAbilityLiveCompositionTests`' two live-press journeys press the HUD button. Deliberately not done: re-pointing `CyberpunkMonsterCrawlUITests` at the now-durable `gameplay.hudPulseButton` anchor to prove PLAY -> `.gameplay` end-to-end (flagged on PR #63 as a cheap win, explicitly not a request - the gap stays recorded as-is)
