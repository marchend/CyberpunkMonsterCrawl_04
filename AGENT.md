...
- Raccoon swarm (`CYBERPUN-17-8`) — **the authoritative implemented-vs-outstanding list for this story; read this rather than inferring the remaining scope from `RaccoonNode`/`RaccoonSeekBehavior`/`RaccoonSpawnDirector`'s doc comments.**
  ...
  Outstanding, no invented ticket IDs: the kill/XP hook's consumer is no longer among them — `CYBERPUN-17-9` (its own entry below) landed it: `Player` sets `RaccoonNode.onDeath` on the first bullet hit, so a raccoon is damaged, killed and counted (`RunStats`) in a real run. Still outstanding: a rabies HUD indicator; the playtesting retune of every named constant; and **the HP-reaches-zero → `.death` transition itself** (`CYBERPUN-17-13`) — `PlayerNode.takeDamage` clamps at 0 and stops, so a player out of HP keeps playing at 0 HP (biteable, still ticking) rather than dying. `CYBERPUN-17-13` PR 2 landed the *content* the death/high-scores screens show once `.death` is reached (see that story's own entry below), but nothing in a real run currently drives the transition itself — today `.death` is reachable only via a direct `stateMachine.transition(to: .death)` call (every test that exercises it, and no production caller). Deliberately left unwired rather than added ad hoc in that PR: `GameScene.advanceMovementAndCamera`'s `.gameplay`-gated block also drives `RaccoonSpawnDirector`/`PickupManager` with a **non-deterministic** RNG (`RaccoonSpawnDirector`'s default `rng: SplitMix64RandomNumberGenerator(seed: UInt64.random(in:))`), and ending a run mid-window would stop those systems' per-frame ticking — several existing scene-wiring suites (`RaccoonSwarmSceneWiringTests`, `PickupIntegrationTests`) drive `GameScene.update(_:)` across a fixed, keyed-to-real-time window and assert on cadence continuing throughout it, so wiring the trigger inside that same loop risks making those suites intermittently flaky depending on how quickly the swarm happens to converge on a stationary player. No ticket ID exists for this yet — file one before the story closes; a human call is needed on whether the trigger belongs inside `advanceMovementAndCamera` (with those suites re-audited/seeded for determinism first) or behind a narrower, explicitly-gated check. **Escalated on `CYBERPUN-17-13-t3` (PR #51):** that PR deleted the DEBUG-only `LaunchGotoState` launch bridge, which until then was the only non-test caller able to reach `.death` at all, so as of it *no build* — DEBUG included — transitions to `.death`. `DeathScreenNode`, `RunScoreCalculator`, `HighScoreStore.recordRun`, `HighScoresScreenNode`'s just-finished-run highlight and `GameScene.startNewRun()` are consequently all unreachable by a player, and the suite cannot see it because every test that exercises `.death` enters it by a direct `stateMachine.transition(to: .death)` call. Recorded at the composition site too (`GameViewController.makeGameScene`, at the `.death` registration). Still NO TICKET ID — file one before this story closes rather than shipping the death/high-scores surface dark
- Final gameplay HUD, death-screen run-summary rows and the high-scores list
  ...
  carry `// SCAFFOLDING(CYBERPUN-17-13)` placeholder content (retagged at PR #49
  from the unfiled -17-16 to the story that removes it). `GameplayScreenNode` no longer carries placeholder *text*:
