...
- High-score persistence data layer landed (`CYBERPUN-17-13-t1`: `RunScoreCalculator`/`HighScoreStore` - bounded top-10 table, deterministic score-desc/sequence-asc ranking, throwing reads/writes (unreadable payloads are quarantined, never overwritten), versioned payload + tolerant entry decoding, failable `init?(suiteName:)`; no UI yet); the SCAFFOLDING marker grep gate; audio, app icon art, launch screen polish, App Store metadata/submission
